...
const workingPhotos = (initialLib.photos || []).map((p: any) => ({ ...p }));
let workingPeople = (initialLib.people || []).map((p: any) => ({ ...p }));
let workingFaces = (initialLib.faces || []).map((f: any) => ({ ...f }));
...
const photosWithFacesRemaining = workingPhotos.filter((p: any) => p.faces && p.faces.length > 0);
assert(photosWithFacesRemaining.length === 0, 'Every photo has its faces array reset to empty');

const photosWithScanCompletedTrue = workingPhotos.filter((p: any) => p.faceScanCompleted === true);
assert(
...
const candidates = workingPhotos.filter((p: any) => {
...
const progressEvents: Array<{ current: number; total: number; currentPhotoName?: string }> = [];
...
if (detectedFaces.length > 0) {
  workingPeople.push({
    ...
    photoCount: workingPhotos.filter((p: any) => p.faces && p.faces.length > 0).length,
...
if (fs.existsSync(mirrorDir)) {
  ...
  const mirrorJpgs = mirrorFiles.filter((f: string) => f.endsWith('.jpg'));
  const mirrorJsons = mirrorFiles.filter((f: string) => f.endsWith('.json'));
