...
export async function detectFacesInImage(
  ...
  try {
    if (typeof source !== 'string') {
      ...
      // Convert filePath to loadable URL (using original high-res photo if preferOriginal is true)
      const url = getLocalPhotoUrl(filePath, originalRemotePath, preferOriginal);
      const img = new Image();
    ...
    return detections.map((det: any, index: number) => {
      ...
      if (det.expressions) {
        ...
        let maxVal = -1;
        for (const [expr, val] of Object.entries(det.expressions)) {
      ...
      // CosFace L2-normalized unit descriptor
